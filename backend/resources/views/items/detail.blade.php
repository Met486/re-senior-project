        ...
        <div class="panel-body">
          ...
          <div id="carouselPhotos" class="carousel slide" data-bs-ride="carousel">
            <div class="carousel-indicators">
              @foreach ($photos as $i)
              <button data-bs-target="#carouselPhotos" data-bs-slide-to="{{ $i->index }}" <?php if ($i->index ==0) echo
                'class="active" aria-current="true"'; ?> aria-label="Slide {{ $i->index+1 }}"></button>
              @endforeach
            </div>
            <div class="carousel-inner">
              @foreach ($photos as $i)
              <div class="carousel-item c-div <?php if ($i->index == 0) echo " active"; ?>" >
                <a a data-bs-toggle="modal" data-bs-target="#lightboxModalFullscreen"
                  data-bs-lightbox="{{ asset( $i->path ) }}" role="button">
          ...
          <div>
            ...
            <input type="submit" value="評価" class="btn btn-primary" a data-bs-toggle="modal"
          ...
          <a href="{{ route('items.addComment', $item->id) }}">
            <input type="button" value="コメント" class="btn btn-primary"
              onclick="location.href={{ route('items.addComment', $item->id) }}">
          </a>
          ...
          <form action="{{ route('items.edit', $item->id) }}" method="get" class="float-right">
            @csrf
            @method('get')
...
<div class="modal" id="lightboxModalFullscreen" tabindex="-1" aria-labelledby="lightboxModalFullscreenLabel"
  aria-hidden="true">
  <div class="modal-dialog modal-fullscreen" data-bs-dismiss="modal" aria-label="Close">
    <div class="modal-content">
      <div class="modal-body d-flex align-items-center justify-content-center">
        <img src="/item/item/photos/_0.png" class="img-fluid" id="LightboxImage" data-bs-dismiss="modal"
          aria-label="Close" />
      </div>
    </div>
  </div>
    ...
    <div class="modal-content bg-body">
      ...
      <div class="modal-body d-flex align-item-center justify-content-center">
        ...
        <div class="row row-cols-3">
          <div class="col">
            <form action="{{ route('evaluation.create', ['id' => $item->id, 'value' => 1]) }}" method="post"
              class="float-right">
              @csrf
          ...
          <div class="col">
            <form action="{{ route('evaluation.create', ['id' => $item->id, 'value' => 2]) }}" method="post"
              class="float-right">
              @csrf
          ...
          <div class="col">
            <form action="{{ route('evaluation.create', ['id' => $item->id, 'value' => 3]) }}" method="post"
              class="float-right">
              @csrf
